...
@section('content')
    <div class="max-w-lg mx-auto px-4 py-12">
        <div class="bg-white rounded-xl shadow-sm border p-8">
            @if(session('otp_kirim'))
                <div x-data="{ timer: 300, display: '05:00', expired: false }" x-init="setInterval(() => { if (timer > 0) { timer--; let m = Math.floor(timer / 60); let s = timer % 60; display = String(m).padStart(2, '0') + ':' + String(s).padStart(2, '0') } else { expired = true } }, 1000)">
                    <h2 class="text-xl font-bold text-gray-800 text-center">Verifikasi Kode OTP</h2>
                    <p class="text-gray-500 text-center mt-2">Kode OTP telah dikirim ke nomor <strong>{{ session('otp_nomor') }}</strong></p>

                    <form method="POST" action="{{ route('otp.verifikasi') }}" class="mt-6 space-y-4">
                        @csrf
                        <input type="hidden" name="no_hp" value="{{ session('otp_nomor') }}">
                        <div>
            ...
            @else
                ...
                <p class="text-gray-500 text-center mt-2">Masukkan nomor HP Anda untuk menerima kode OTP.</p>

                <form method="POST" action="{{ route('otp.kirim') }}" class="mt-6 space-y-4">
                    @csrf
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nomor HP</label>
                        <input type="text" name="no_hp" value="{{ old('no_hp', session('otp_nomor')) }}" required placeholder="08xxxxxxxxxx"
                    ...
                    <div>
                        ...
                        <select name="jenis_pemohon" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none">
                    ...
                    <div id="field-instansi" x-data="{ show: '{{ old('jenis_pemohon', 'publik') }}' }" x-show="show === 'instansi'">
                    ...
                    <button type="submit" class="w-full bg-primary-500 text-white py-3 rounded-lg font-semibold hover:bg-primary-600 transition">Kirim OTP</button>
                </form>
            @endif
        </div>
    </div>
...
@section('scripts')
<script>
    document.querySelector('[name="jenis_pemohon"]')?.addEventListener('change', function() {
        const instansi = document.getElementById('field-instansi');
        if (instansi) {
            const alpineInstance = instansi.__x;
            if (alpineInstance) alpineInstance.$data.show = this.value;
        }
    });
</script>
@endsection